function ahSlider(sliderId, content, options = {}) {
  ...
  /* Отримуєм об`єкт слайдера */
  let slider = document.querySelector(`#${sliderId}`);

  /* Створює слайди в слайдері */
  for (let x = 0; x < content.length; x++) {

    /* Створює елемент div контейнер для слайду */
    const slide = document.createElement("div");
    slide.classList.add("ah-slider_item");

    /* Створює елемент img для слайду */
    const image = document.createElement("img");
    image.setAttribute("src", content[x].image);
    image.setAttribute("alt", `${x + 1} slide`);
    slide.append(image)

    /* Створює елемент заголовку слайду */
    const title = document.createElement("div");
    title.classList.add("ah-slider_item__title");
    title.innerHTML = content[x].title;
    slide.append(title);

    /* Створює елемент тексту слайду */
    const text = document.createElement("div");
    text.classList.add("ah-slider_item__text");
    text.innerHTML = content[x].text;
    slide.append(text);
